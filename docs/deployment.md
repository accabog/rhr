...
After each staging deployment, demo credentials are automatically seeded:
...
**Tenants:** demo-acme, demo-techcorp, demo-global

The seeding runs with `--force --no-holidays` flags:
- `--force` - Required for production settings module
- `--no-holidays` - Skips external API calls that could timeout

To manually seed staging data:
```bash
docker compose -f docker-compose.prod.yml exec backend python manage.py seed_staging_data --force --no-holidays
```
