...
import type { AuthResponse, GoogleAccount, LoginCredentials, RegisterData, User } from '@/types';
...
export const authApi = {
  ...
  connectGoogle: async (credential: string): Promise<GoogleConnectResponse> => {
    const response = await apiClient.post<GoogleConnectResponse>('/users/me/google/connect/', { credential });
    return response.data;
  },

  disconnectGoogle: async (): Promise<void> => {
    await apiClient.post('/users/me/google/disconnect/');
  },
};
